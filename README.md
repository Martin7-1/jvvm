...
> 其中，`PrintClassLoader`是我们自己写的类，`Logging`是第三方类库（<JAVA_HOME>/lib(ext)目录下），`ArrayList`是Java自带的库。
...
2. Extension Classloader负责加载标准Java库以外的扩展库，即<JAVA_HOME>/lib下的第三方库，又或者说是扩展类路径（Extension Classpath）
